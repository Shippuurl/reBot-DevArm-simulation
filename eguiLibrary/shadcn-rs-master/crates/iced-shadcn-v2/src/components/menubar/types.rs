//! Configuration and entry types used by the menubar component.

use std::fmt;

use shadcn_common::{MenuActivateKind, MenuItemVariant};

/// Visual variant of a menubar item (`data-variant`).
///
/// Alias of [`MenuItemVariant`] from `shadcn-common` so iced consumers can
/// import it from this crate.
pub type MenubarItemVariant = MenuItemVariant;

/// One top-level menubar menu (`Menubar.Menu` + `Trigger` + `Content`).
#[derive(Clone)]
#[must_use = "menus do nothing unless pushed into a Menubar"]
...
impl<Message> fmt::Debug for MenubarMenu<Message> {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("MenubarMenu")
...
impl<Message> MenubarMenu<Message> {
    /// Creates an empty menu with the given trigger label.
    ...
    /// Appends a plain item (`Menubar.Item`).
    pub fn item(mut self, item: MenubarItem<Message>) -> Self {
        self.entries.push(Entry::Item(item));
        self
    }

    /// Appends a checkbox item (`Menubar.CheckboxItem`).
    pub fn checkbox_item(mut self, item: MenubarCheckboxItem<Message>) -> Self {
        self.entries.push(Entry::Checkbox(item));
        self
    }

    /// Appends a radio item (`Menubar.RadioItem`).
    pub fn radio_item(mut self, item: MenubarRadioItem<Message>) -> Self {
        self.entries.push(Entry::Radio(item));
        self
    }

    /// Appends a section label (`Menubar.Label`).
    pub fn label(mut self, label: impl Into<MenubarLabel>) -> Self {
        self.entries.push(Entry::Label(label.into()));
        self
    }

    /// Appends a hairline separator (`Menubar.Separator`).
    pub fn separator(mut self) -> Self {
        self.entries.push(Entry::Separator);
        self
    }

    /// Appends a nested submenu (`Menubar.Sub`).
    pub fn submenu(mut self, submenu: MenubarSub<Message>) -> Self {
    ...
    /// Whether the menu has no content entries.
    #[must_use]
...
/// One plain menu action (`Menubar.Item`).
#[derive(Clone)]
#[must_use = "items do nothing unless pushed into a MenubarMenu"]
pub struct MenubarItem<Message> {
...
impl<Message> fmt::Debug for MenubarItem<Message> {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("MenubarItem")
...
impl<Message> MenubarItem<Message> {
    ...
    /// Sets the keyboard shortcut hint (`Menubar.Shortcut`).
...
/// One checkbox row (`Menubar.CheckboxItem`).
#[derive(Clone)]
#[must_use = "checkbox items do nothing unless pushed into a MenubarMenu"]
pub struct MenubarCheckboxItem<Message> {
    pub(super) label: String,
...
impl<Message> fmt::Debug for MenubarCheckboxItem<Message> {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("MenubarCheckboxItem")
            .field("label", &self.label)
...
impl<Message> MenubarCheckboxItem<Message> {
...
/// One radio row (`Menubar.RadioItem`).
#[derive(Clone)]
#[must_use = "radio items do nothing unless pushed into a MenubarMenu"]
pub struct MenubarRadioItem<Message> {
    pub(super) label: String,
...
impl<Message> fmt::Debug for MenubarRadioItem<Message> {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("MenubarRadioItem")
            .field("label", &self.label)
...
impl<Message> MenubarRadioItem<Message> {
...
/// Non-interactive section heading (`Menubar.Label` / `GroupHeading`).
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
#[must_use = "labels do nothing unless pushed into a MenubarMenu"]
...
/// Nested submenu (`Menubar.Sub` + `SubTrigger` + `SubContent`).
#[derive(Clone)]
#[must_use = "submenus do nothing unless pushed into a MenubarMenu"]
pub struct MenubarSub<Message> {
    pub(super) label: String,
    pub(super) disabled: bool,
    pub(super) inset: bool,
    pub(super) entries: Vec<Entry<Message>>,
...
impl<Message> fmt::Debug for MenubarSub<Message> {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("MenubarSub")
            .field("label", &self.label)
            .field("disabled", &self.disabled)
            .field("inset", &self.inset)
            .field("entries", &self.entries.len())
...
impl<Message> MenubarSub<Message> {
...
impl From<&str> for MenubarLabel {
    fn from(text: &str) -> Self {
        Self::new(text)
    }
}

impl From<String> for MenubarLabel {
    fn from(text: String) -> Self {
        Self::new(text)
    }
}

/// One top-level or nested menu entry owned by a [`MenubarMenu`].
#[derive(Clone)]
pub(super) enum Entry<Message> {
    Label(MenubarLabel),
    Separator,
    Item(MenubarItem<Message>),
    Checkbox(MenubarCheckboxItem<Message>),
    Radio(MenubarRadioItem<Message>),
    Sub(MenubarSub<Message>),
}
