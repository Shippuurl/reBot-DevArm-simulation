//! Custom widget and overlay rendering for [`super::DropdownMenu`].
//!
//! The widget wraps a trigger element. While open, an iced overlay paints the
//! `.cn-dropdown-menu-content` surface with rows (items, checkboxes, radios,
//! labels, separators, sub-triggers) and optional nested
//! `.cn-dropdown-menu-sub-content` panels. Keyboard navigation matches bits-ui
...
use shadcn_common::{
    Direction, DropdownMenuRecipe, MENU_SUB_SIDE_OFFSET_PX, MenuItemVariant, NavAction, NavKey,
    Orientation, resolve_nav_action,
};
...
use super::style::{self, DropdownMenuContentStyle, item_colors, item_highlight_fill};
use super::types::Entry;
use crate::fonts::iced_font;
use crate::recipes::iced_font_weight;
use crate::theme::Theme;

/// Internal widget produced by the [`super::DropdownMenu`] builder.
pub(super) struct DropdownMenuWidget<'a, Message> {
    ...
    pub(super) style_override:
        Option<Box<dyn Fn(DropdownMenuContentStyle) -> DropdownMenuContentStyle + 'a>>,
}

/// Widget-tree state of the trigger and its menus.
struct State {
    requested_open: bool,
...
impl<Message> DropdownMenuWidget<'_, Message>
where
    Message: Clone,
{
    ...
    fn resolve_style(&self, submenu: bool) -> DropdownMenuContentStyle {
    ...
    fn handle_trigger_press(&self, state: &mut State, shell: &mut Shell<'_, Message>) {
        ...
        let next = !self.is_open(state);
        if next {
            state.hovered = self.entries.iter().position(Entry::is_selectable);
            state.open_path.clear();
            state.hovered_sub = None;
        }
        self.set_open(state, next, shell);
    }
}

impl<'a, Message> Widget<Message, IcedTheme, Renderer> for DropdownMenuWidget<'a, Message>
where
    Message: Clone + 'a,
{
        ...
        match event {
            ...
            | Event::Touch(touch::Event::FingerPressed { .. }) => {
                ...
                if cursor.is_over(layout.bounds()) {
                    ...
                    // Outside press closes unless the overlay captures it first.
                    // Overlay runs after widgets in some paths; use suppress when
                    // the overlay itself handled the press.
                    state.suppress_next_trigger_press = false;
    ...
    fn overlay<'b>(
        ...
        let width = self
            .width
            .unwrap_or(trigger_bounds.width)
            .max(recipe.content_min_width_px)
            .max(trigger_bounds.width);

        Some(
            MenuOverlay {
                position: Point::new(trigger_bounds.x, trigger_bounds.y),
...
struct MenuOverlay<'a, Message> {
    ...
    content_style: DropdownMenuContentStyle,
    sub_style: DropdownMenuContentStyle,
    item_font: Font,
    label_font: Font,
    shortcut_font: Font,
