//! Mapping of `.cn-context-menu-*` style-pack rules to resolved iced visuals.
//!
//! The context-menu surface and items share the `.cn-menu-target` /
//! `.cn-menu-translucent` CSS with the dropdown-menu, so the resolution is
//! identical. We expose a [`ContextMenuContentStyle`] alias plus
//! [`resolve_content_style`] that reads the [`ContextMenuRecipe`] and the
//! context-menu destructive-focus alphas so a future split stays local.

use crate::iced_compat::{Color, Shadow, Vector};

use shadcn_common::{
    CONTEXT_MENU_DESTRUCTIVE_FOCUS_ALPHA, CONTEXT_MENU_DESTRUCTIVE_FOCUS_ALPHA_DARK,
    CONTEXT_MENU_DISABLED_OPACITY, ContextMenuRecipe, MenuItemVariant,
};
...
pub struct ContextMenuContentStyle {
...
/// `.cn-context-menu-*` numbers of the active pack.
pub(super) fn recipe(theme: &Theme) -> ContextMenuRecipe {
    theme.style.context_menu()
}

/// Resolves the content / item palette for the active theme.
pub(super) fn resolve_content_style(theme: &Theme, submenu: bool) -> ContextMenuContentStyle {
    ...
    let destructive_focus_alpha = if theme.is_dark() {
        CONTEXT_MENU_DESTRUCTIVE_FOCUS_ALPHA_DARK
    } else {
        CONTEXT_MENU_DESTRUCTIVE_FOCUS_ALPHA
    };
    ...
    ContextMenuContentStyle {
        ...
        separator_color,
        item_disabled_opacity: CONTEXT_MENU_DISABLED_OPACITY,
