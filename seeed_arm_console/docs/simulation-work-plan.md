...
构建一套不依赖 MoveIt 的 reBot-DevArm 仿真工作站：Pinocchio + ProxSuite 负责规划，MuJoCo 负责动力学仿真，Rerun 负责统一观察、记录和回放；外部工程只通过官方 SDK 访问 gRPC 服务。
...
- 新增 `scripts/planner_gateway_smoke.py` 与 `scripts/run_planner_gateway_smoke.sh`，打通 `PlanTrajectory` → ArmGateway dry-run → 实际 MuJoCo 执行 → `SubscribeTelemetry` 回读 → Stop/ResetFault；MATE 阶段以 0.001 m 余量验证规划候选可安全交接。
...
- Rerun Viewer 已增加按字节计量的有界历史缓存：默认 `256MiB`，达到预算后由 gRPC proxy 和 Viewer 优先淘汰最早动态 chunk，静态模型优先保留；`RERUN_DEBUG_MODE=1` 默认切换为 `64MiB`/`30Hz`，也可用 `RERUN_HISTORY_LIMIT` 与 `RERUN_TELEMETRY_RATE_HZ` 单独覆盖。
...
| ArmGateway gRPC | 可运行 | C++ 服务已绑定 `SimulationDriver`；50051 提供握手、控制、轨迹执行、故障复位和遥测，50052 保留 JSON 兼容端口 |
...
| 碰撞检查 | 原型可用 | Coal 几何已加载；已过滤相邻 link 和夹爪装配接触，并返回碰撞对名称/最近距离；规划默认 0.02 m、监控底线 0.01 m，MATE 无显式覆盖时 0.001 m，支持请求级 ACM 白名单 |
...
| ArmGateway 正式 gRPC | 仿真链路完成 | gRPC 已成为默认链路；协议版本、独立 session（1 小时 TTL、最多 1024 个）、命令时间戳新鲜度、Jog/轨迹安全边界、限位、dry-run、实际 MuJoCo 执行、Pause/Resume、SpeedScale、ResetFault 及 Planner→Gateway 联调已接入；真实设备执行适配、硬件 watchdog 和真机链路断连恢复仍待独立安全审核 |
...
4. ~~接入完整碰撞几何，过滤相邻 link 的允许接触并报告碰撞对名称和最近距离。~~ 已完成请求级 ACM 白名单、规划默认 0.02 m、监控底线 0.01 m 和 MATE 阶段 0.001 m 覆盖；设备力/力矩 watchdog 仍待接入。
...
- `50052`：换行 JSON legacy adapter，供 `mujoco_rerun_bridge` 和旧联调脚本使用；设置 `ARM_CONSOLE_ENABLE_JSON=0` 可关闭。
